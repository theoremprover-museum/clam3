/************************************************************************
      ...
      File: data_type_ch.pl
      Author: Louise Dennis
      Last Modified: 31st July 1997

      static semantics, and transition rules for a functional
      programming language in which tree, list and integer types are
      all lazy, and typing is defined coinductively
...
%% integers
transition([], 0, 0, bot).
transition([], type(0, pnat), 0, type(bot, bot)).
transition([], s(N), p, N).
transition([], type(s(N), pnat), p, type(N, pnat)).
...
data:-consult('~/xclam/data_type_ch.pl').
