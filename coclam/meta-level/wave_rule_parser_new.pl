...
annotate(T,AL,AR)  :-       % return admissible annotations
...
pick_an(T,T) :- atomic(T), !.  % Don't annotate
pick_an(T,AT) :-  T =.. [F|Args],  mappredlist(pick_an, Args, AnArgs),   AT =.. [F|AnArgs].
pick_an(T,AT) :-   T =.. [F|Args],  pick_an(Args,AnArgs,0), AnT =.. [F|AnArgs], box(AnT,AT).

% stick a hole around at least one subterm
pick_an([],[],N) :- N > 0.
pick_an([H|T1],[H|T2],N) :- pick_an(T1,T2,N).
pick_an([H1|T1],[H2|T2],N) :-  pick_an(H1,AH1), hole(AH1,H2), NN is N+1, pick_an(T1,T2,NN).
...
% skeletons(T,Skelset) :- unannotated(T), Skelset = [T], !.    % Unannotated --- return self

% skeletons(T,Skelset) :-                    % Wave front case
%   wfholes(T,Holes),                        % list of holes
%   mappredlist(skeletons, Holes, HoleSkels),    % skel sets for each hole
...
%    T =.. [F|Args],
%    mappredlist(skeletons, Args, Skels), 
%    setprod(Skels,FArgs),
%    mappredlist(args_to_func(F), FArgs, Skelset).
...
% weaken(T,Q) :- T =.. [F|Args],  mappredlist(weaken, Args, WArgs), Q =.. [F|WArgs].
...
measure(Dir,T,M) :-  weakenings(T,TWS),  mappredlist(simple_measure(Dir),TWS,M).
...
%   T =.. [_|Args],
%   mappredlist(simple_measure(Dir),Args,ArgVals),      % get measures for arguments
...
% added to eliminate wave-rules which position wave-holes directly
% on constants.
...
% erase(T,Q) :- T =.. [F|Args],  mappredlist(erase, Args, EArgs), Q =.. [F|EArgs].
...
%    T1 =.. ExpT1, mappredlist(map_on_tree(Pred),ExpT1,ExpT2), T2 =.. ExpT2.
...
%
mappredlist(_, [], []).
mappredlist(Pred, [A|AList], [NewA|NewAList]):-
        atomic(Pred),!,
	T =.. [Pred, A, NewA],
	call(T),
	mappredlist(Pred, AList, NewAList).
mappredlist(PredArg, [A|AList], [NewA|NewAList]):-
        PredArg =.. [Pred, Arg],
	T =.. [Pred, Arg, A, NewA],
	call(T),
	mappredlist(PredArg, AList, NewAList).
