...
refine_goal(Name, Addr, Mthd, N, DepthBound):-      % iterative deepening 
    ...
    current_node(Name, NAddr),
    refine_goals(Name, NAddr, N, DepthBound),!.
refine_goal(Name, Addr, Mthd, N, DepthBound):-      % backtrack to nearest
    nearest_choice_point(Name, Addr, NAddr),        % (method) choice point
    move_to_node(Name, NAddr),
    current_method(Mthd),
    current_preconds(Preconds),
    current_successful_preconds([Pre1,Pre2|_]),
    remove(Pre1, Preconds, NPreconds),
    delete_branch(Name, NAddr),
    record_preconds(Name, NAddr, NPreconds),
    skeleton(Mthd, Method),
    writef('\nBacktracking to %t (choice point)\n', [NAddr]),
    expand_goal(Name, NAddr, Method, Pre2, N),!,
    refine_goals(Name, NAddr, N, DepthBound).
/*
refine_goal(Name, Addr, Mthd, N, DepthBound):-      % backtrack to nearest
    nearest_patch_point(Name, Addr, NAddr, _),      % (method) patch point
    move_to_node(Name, NAddr),
    current_method(Mthd),
    current_preconds([Pre|Preconds]),
    delete_branch(Name, NAddr),
    record_preconds(Name, NAddr, Preconds),
    skeleton(Mthd, Method),
    writef('\nBacktracking to %t (patch point)\n', [NAddr]),
    expand_goal(Name, NAddr, Method, Pre, NSG),!,
    refine_goals(Name, NAddr, NSG, DepthBound).
*/
refine_goal(_, Addr, _, _, _):-
    length(Addr, Depth),
    writef('\nFailed at depth %t\n', [Depth]),!,fail.
...
apply_method(ConjName, Method, Name, Arity, Pre, NumSGs):-
    ...
    node(ConjName, Addr, Input, _, _, _, _, _),
...
eval_mthd_preconds(Mthd, _, Thm, Addr):-      % Pre requires evaluation
    ...
    fail.
eval_mthd_preconds(_, Pre, Thm, Addr):-       % Select next instantiation
    node(Thm, Addr, _, _, _, _, Preconds, _), % for Pre from recorded
    elim_and_choices(Preconds, NPreconds),    % preconditions
    member(Pre, NPreconds),
    \+ member(false, Pre).

elim_and_choices(PreListAll, NewPreList):-
        findall(P, (member(P, PreListAll), 
                    \+ member(false, P)), SPreListAll),
        member([wave_occ_at(_, _, _),
                wave_rule_match(_, _, _, _),_|_], SPreListAll),
...
%    \+ member(Critic, [wave(split), dwave(join), 
%                       wave(meta-ripple-dir),  
%                       wave(meta-ripple-pos)]),
