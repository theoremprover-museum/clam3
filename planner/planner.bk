...
refine_goal(Name, Addr, dplan):-
    refine_goal(Name, Addr, _, _, _).
refine_goal(Name, Addr, idplan):-
    bound(DepthBound),
    writef('Depth bound set to %t\n', [DepthBound]),
    delete_branch(Name, Addr),
    refine_goal(Name, Addr, _, _ , DepthBound).

    % refine_goal/5
    %
    %
refine_goal(Name, Addr, Mthd, N, DepthBound):-
    var(DepthBound),
    expand_goal(Name, Addr, Mthd, _, N),
    current_node(Name, NAddr),
    refine_goals(Name, NAddr, N, DepthBound),!.
refine_goal(Name, Addr, Mthd, N, DepthBound):-
    \+ var(DepthBound),
    length(Addr, Depth),
    ((Depth < DepthBound); non_speculative_goal(Name, Addr)),!,
    expand_goal(Name, Addr, Mthd, _, N),
    current_node(Name, NAddr),
    refine_goals(Name, NAddr, N, DepthBound),!.
refine_goal(Name, Addr, Mthd, N, DepthBound):-
    ...
    node(Name, NAddr, _, _, Mthd, _, PreL, _),
    skeleton(Mthd, Method),
    findall(P, (member(P, PreL), \+ member(false, P)), [_,Pre|NewPreL]),
    apply(method, [Method, Input, Pre, Effects, Subgoals, SubstL]),
    try_postconditions(Effects),
    map_list(Subgoals, SG:=>NSG, 
                       apply_annsubst_list(SubstL, SG, NSG), 
             NSubgoals),
    delete_branch(Name, NAddr),
    extend_goaltree(Name, NAddr, Method, SubstL, NSubgoals, NumSGs),
    record_preconds(Name, NAddr, [Pre|NewPreL]),
    writef('\nFailed at depth %t, backtracking to %t\n', [Depth,NAddr]),
    display_sequent(Name, NAddr),
    display_method(Method, NAddr, NumSGs),!,
    refine_goals(Name, NAddr, NumSGs, DepthBound),!.
refine_goal(Name, Addr, Mthd, N, DepthBound):-
    length(Addr, Depth),
    writef('\nFailed at depth %t\n', [Depth]),!,fail.
...
refine_goals(Name, Addr, N, DepthBound):-
    N > 0,
    refine_goal(Name, [N|Addr], _, _, DepthBound),
    M is N-1,
    refine_goals(Name, Addr, M, DepthBound).

    % expand_goal/5
    %
    %
expand_goal(Name, Addr, _, _, 0):-
    node(Name, Addr, _, closed, _, _, _, _),!.
expand_goal(Name, Addr, _, _, 0):-
...
apply_method(ConjName, Method, Name, Arity, Pre, NumSGs):-
    ...
    apply(method,[Method,Input,Pre,Effects,Subgoals,SubstL]),
    % writef('Evaluating method preconditions %t\n',[Method]),
    eval_mthd_preconds(Method, Pre, ConjName, Addr),
    % writef('Evaluating method effects %t\n',[Method]),
    try_postconditions(Effects),
    % writef('SubstL generated by %t is: %t\n\n',[Method, SubstL]),
    map_list(Subgoals, SG:=>NSG, 
                       apply_annsubst_list(SubstL, SG, NSG), 
             NSubgoals),
    ...
    %
apply_method(ConjName, Method, Name, Arity, _, NumSGs):-
    recorded(Name,critic(_,_),_),
    % writef('\n>>>>> CHECKING PRE FOR %t CRITIC ',[Name]),nl,
    current_node(ConjName, Addr),
    node(ConjName, Addr, Input, _, _, _, EvalMthdPre, _),
    % writef('\n>>>>> EvalMthdPre = %t ', [EvalMthdPre]),
    apply(critic,[Name,ConjName,Preprocess,Preconds,Effects]),
    % writef('Evaluating critic preconditions %t\n',[Name]),
    try_postconditions(Preprocess),
    try_postconditions(Preconds),
    writef('\n>>>>> INVOKING %t CRITIC ',[Name]),
    call_conjunction(Effects),
    current_node(ConjName, NewAddr),
    % record_preconds(ConjName, NewAddr, _),
    % First check to see if the critic has completed the
    % current branch.
    ((closed_node(ConjName, NewAddr),NumSGs = 0);
     apply_method(ConjName, Method, Name, Arity, _, NumSGs);
    (current_node(ConjName, Addr),
     record_preconds(ConjName, Addr, []),!,fail)).

    % eval_mthd_preconds/4
    %
    % Prototype code for supporting partial evaluation of
    % method preconditions (currently too many permuatations
    % of precondition failures are returned).
    %
eval_mthd_preconds(Mthd, Pre, Thm, Addr):-
    functor(Mthd, Name, _),
    \+ recorded(Name,critic(_,_),_),!,
    node(Thm, Addr, Input, _, _, _, _, _),
    apply(method,[Mthd,Input,Pre,Effects,Subs,_]),
    call_conjunction(Pre),
    record_preconds(Thm, Addr, [Pre]).
eval_mthd_preconds(Mthd, Precond, Thm, Addr):-
    node(Thm, Addr, Input, _, _, _, _, _),
    apply(method,[Mthd,Input,Pre,Effects,Subs,_]),
    eval_preconds(Pre, EvalPre),
    update_preconds(Thm, Addr, EvalPre),
    fail.
eval_mthd_preconds(Mthd, Precond, Thm, Addr):-
    node(Thm, Addr, _, _, _, _, Preconds, _),
    elim_and_choices(Preconds, NPreconds),
    member(Precond, NPreconds).
    \+ member(false, Precond).

elim_and_choices(PreListAll, NewPreList):-
        findall(P, (member(P, PreListAll), \+ member(false, P)), SPreListAll),
        member([wave_occ_at(_, _, _),wave_rule_match(_, _, _, _),_|_], SPreListAll),
	elim_and_choices_(SPreListAll, NewPreList),!.
...
elim_occs(Pos, [H|T], [H|NenwT]):-
    ...
    %
eval_preconds([],[]).
eval_preconds([P|Q], CompletePs1):-
    findall(EP-[NewP|NewQ],
            (copy([P|Q],[NewP|NewQ]),call_pre(P,EP)),
            PartialPs),
    eval_precond_lists(PartialPs, CompletePs1).

determine_status([P|_],partial):-
    member(false, P),!.
determine_status(_, complete).

eval_precond_lists([], []).
eval_precond_lists([false-[_]|Ps], [[false]|EvalPs]):-!,
    eval_precond_lists(Ps, EvalPs).
eval_precond_lists([EvalP-[EvalP]|Ps], [[EvalP]|EvalPs]):-!,
    eval_precond_lists(Ps, EvalPs).
eval_precond_lists([false-[EvalP|NewQ]|Ps], EvalNewPsQs):-!,
    eval_preconds(NewQ, EvalNewQs),
    map_list(EvalNewQs,
            EvalNewQ:=>EvalNewPQ,
            append([false],EvalNewQ,EvalNewPQ),
            EvalNewPQs),
    eval_precond_lists(Ps, EvalPs),
    append(EvalNewPQs, EvalPs, EvalNewPsQs).

eval_precond_lists([EvalP-[EvalP|NewQ]|Ps], EvalNewPsQs):-
    eval_preconds(NewQ, EvalNewQs),
    map_list(EvalNewQs,
            EvalNewQ:=>EvalNewPQ,
            append([EvalP],EvalNewQ,EvalNewPQ),
            EvalNewPQs),
    eval_precond_lists(Ps, EvalPs),
    append(EvalNewPQs, EvalPs, EvalNewPsQs).
...
try_postconditions(L):- call_conjunction(L),!.
...
apply_subst_to_node(Name, Addr, Subst):-
    ...
    assert(node(Name, Addr, HH==>GG, Status, Mthd, SL, Pre, N)),
...
init_goaltree(Name):-
    ...
    select(Name),
    goal(G), hyp_list(H),
    assert(node(Name, [], H==>G, open, undef, [], [], 0)),
    assert(goal_node(Name, [])),
...
extend_goaltree(Name, Addr, Mthd, SubstL, [], 0):-
    ...
    retract(node(Name, Addr, Goal, open, _, _, Preconds, 0)),
    asserta(node(Name, Addr, Goal, closed, Mthd, SubstL, Preconds, 0)).
extend_goaltree(Name, Addr, Mthd, SubstL, Subgoals, Num):-
    retractall(goal_node(Name, Addr)),
    retract(node(Name, Addr, Goal, open, _, _, Preconds, 0)),
    assert_subgoals(Name, Addr, 0, Subgoals, Num),
    asserta(node(Name, Addr, Goal, closed, Mthd, SubstL, Preconds, Num)).
...
add_node(Name, Addr, Goal, Status, Mthd, SubstL, Pre, N):-
    assert(node(Name, Addr, Goal, Status, Mthd, SubstL, Pre, N)).
...
assert_subgoals(Name, Addr, TotSoFar, [G|Gs], Tot):-
    N is TotSoFar + 1,
    asserta(goal_node(Name, [N|Addr])),
    asserta(node(Name, [N|Addr], G, open, undef, [], [], 0)),
...
update_preconds(Name, Addr, Preconds):-
    ...
    asserta(node(Name, Addr, Goal, open, _, SubstL, NewPreconds, 0)),!.
...
record_preconds(Name, Addr, Preconds):-
    ...
    asserta(node(Name, Addr, Goal, Status, Mthd, SubstL, Preconds, N)).
...
record_newgoal(Name, Addr, NewGoal):-
    retract(node(Name, Addr, _, _, _, _, _, _)),
    asserta(node(Name, Addr, NewGoal, open, [], [], _, 0)).
...
delete_branch(Name, Addr):-
    ...
    assert(goal_node(Name, Addr)),
    undo_list(Name, N, Addr).
...
undo(Name, Addr):-
    retract(node(Name, Addr, Goal, closed, _, _, _, N)),
    undo_list(Name, N, Addr).
...
nearest_choice_point(Plan, Addr, Addr):-
    ...
    findall(P, (member(P, PreL), \+ member(false, P)), SPreL),        
    length(SPreL, N),
    N > 1.
...
ap(N):-
    ...
    delete_branch(Thm, Addr),
    expand_goal(Thm, Addr, Mthd, Precond, _).
