...
refine_goal(Name, UpperBound, dplan):-
    set_planner(dplan),
    refine_goal_(Name, 300, UpperBound).
...
refine_node(Name, LowerBound, UpperBound):-   % backtrack to nearest
    planner(dplan),                           % (critic) choice point
    current_node(Name, Addr),
    ...
    current_method(Method),
    skeleton(Method, Mthd),
    % Deletes branch but leaves precondition list in place
    % so that critics can exploit partial results.
    node(Name, NAddr, Goal, _, _, _, [_|PreL], N),
    add_node(Name, NAddr, Goal, open, undef, [], PreL, 0),
    delete_branches(Name, N, NAddr),
    retract(open_nodes(Name, OList)),
    union([NAddr], OList, NewOList),
    assert(open_nodes(Name, NewOList)),
    delete_lemmas,
...
% update_lemmas/1
%
% Only if the lemma is being used as the 
% basis for a wave-rule schema, i.e. not
% a generalization.
%
update_lemmas(SubstL):-
        ...
        add_thm(Name, []==>Lemma2),
        record_thm(Name, thm),
        writef('\n>>>>> INSTANTIATING %t WAVE-RULE SCHEMA <<<<<\n',[Name]),
        add_wave_rules(Name, _),nl.
update_lemmas(_).
...
no_hou_subst([_,wave_rule_match(_,_,_,[])|_]).
no_hou_subst([partial_induction_hyp_match(_, _, _, _, [])]).

no_pre_failures([]).
no_pre_failures([Pre|PreL]):-
        \+ Pre = false,
        no_pre_failures(PreL).
...
update_preconds(Name, Addr, Preconds):-
    ...
    % append(Preconds, PreSoFar, NewPreconds1),
    remove_dup_pre(NewPreconds1, NewPreconds),
    ...
    %
remove_dup_pre([], []).
remove_dup_pre([H|T], NewT):-
	member(H, T),
        remove_dup_pre(T, NewT).
remove_dup_pre([H|T], [H|NewT]):-
        \+ member(H, T),
        remove_dup_pre(T, NewT).
...
delete_current_rec(_).
    ...
    % chop_branch/2
    %
    % Same as delete_branch/2 but maintains preconditions
    %
chop_branch(Name, Addr):-
    new_current_node(Name, Addr),
    node(Name, Addr, Goal, _, _, _, PreL, N),
    add_node(Name, Addr, Goal, open, undef, [], PreL, 0),
    delete_branches(Name, N, Addr),
...
csp:-
    current_preconds(PL),
    findall(P, (member(P, PL), no_pre_failures(P)), SPL),
...
ap(N):-
    ...
    current_node(Thm, Addr),
    node(Thm, Addr, _, _, _, _, Preconds, _),
    findall(P, (member(P, Preconds), no_pre_failures(P)), SPreconds),
...
options:-
    ...
    write('sg(N): Select goaltree N.'),nl,
    write('ap(N): Apply Nth successful precondition.'),nl,
    write('am(M): Apply method M.'),nl,
    write('dp(N): Display plans for goaltree N.'),nl,
    write('ss(N): Display snapshot of goaltree N.'),nl,
    write('ig(N): Initialize goaltree N.'),nl.
